...
export function ClaimFlow({ createWorker = defaultCreateWorker }: ClaimFlowProps = {}) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: one-time wallet discovery on mount; selectedImpactedWallet is only read to avoid duplicating the default pick and must not re-trigger discovery
  useEffect(() => {
    if (fixtureEnabled) {
      return;
    }
    ...
    setSelectedSafeWallet(
      (current) =>
        current || nextWallets.find(([id]) => id !== selectedImpactedWallet)?.[0] || nextWallets[0]?.[0] || "",
    );
  }, [fixtureEnabled]);
...
function CreateProofs({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: mode and recoveryPhraseWordCount are deliberate extra deps — the word grid inputs are re-rendered when they change and the statuses must be recomputed from the DOM
  useEffect(() => {
        ...
        <Panel title="Impacted wallet recovery phrase" className="claim-phrase-panel">
          ...
          <div className="claim-phrase-grid">
            {Array.from({ length: recoveryPhraseWordCount }, (_, index) => (
              <input
                // biome-ignore lint/suspicious/noArrayIndexKey: fixed-size word grid; the slot position is the identity
                key={index}
...
function ClaimReview({
  ...
  const claimedCount = progress?.outrefs.filter(
    (entry) => entry.state === "spent_or_unknown" || entry.state === "confirmed_spent",
  ).length;
  const totalCount =
            ...
            ? [
                {
                  icon: Coins,
                  label: "Claimed UTxOs",
                  value: `${claimedCount ?? (fixtureMode && pending ? 16 : totalCount)} of ${totalCount}`,
                },
              ]
            : []),
      ...
      <div className="claim-table-footer">
        ...
        <nav className="claim-pagination" aria-label="Claims pages">
          ...
          {paginationItems(page, pageCount).map((item, index) =>
            item === "ellipsis" ? (
              // biome-ignore lint/suspicious/noArrayIndexKey: ellipsis markers have no identity beyond their position
...
export function selectClaimBatchRows(
  ...
  const configuredHardCap =
    deployment.deployment.batching?.hard_max_utxo_count ??
    (statementBoundV2 ? CLAIM_HARD_BATCH_CAP : CLAIM_LEGACY_OPTIMIZATION_BATCH_CAP);
  const hardCap = Math.min(configuredHardCap, statementBoundV2 ? CLAIM_HARD_BATCH_CAP : CLAIM_LEGACY_HARD_BATCH_CAP);
  const selectedCap =
